...
def evaluater(x_test, y_test, model, path):
    ...
    C = confusion_matrix(y_test, y_pred_idx, labels=[0, 1])
    ...
    plt.ylabel('True label (0=Normal, 1=Abnormal)')
    plt.xlabel('Predicted label')
    plt.savefig(path, bbox_inches='tight', pad_inches=0.1, dpi=200)
    plt.close()
    return np.array(cal_metrics(C)), acc

class H5DataGeneratorBinary(tf.keras.utils.Sequence):
    def __init__(self, file_path, batch_size=32, shuffle=True, **kwargs):
    ...
    def __getitem__(self, index):
        ...
        # Binary target: 0 for Normal, 1 for Anomalies (labels > 0)
...
def train_model(model, train_data, val_data, x_test_list, y_test_list, save_path, EP, LR, BS, pic_path_test, pic_path_val):
    ...
    for i in range(len(metrics)):
        ...
        print(f"{label:<10} | {p:<8.4f} | {r:<8.4f} | {s:<8.4f} | {f1:<8.4f}")
    print("-" * 60)
...
if __name__ == '__main__':
    ...
    with h5py.File(dataset_path_test, 'r') as f:
        ...
        y_test = np.where(f['Y'][:] > 0, 1, 0)
        
    with h5py.File(dataset_path_val, 'r') as f:
        x_val_raw = f['X'][:, :6, :]
        x_val_eval = np.transpose(x_val_raw, (0, 2, 1))
        y_val_eval = np.where(f['Y'][:] > 0, 1, 0)

    input_shape = (SAMPLES_PER_WINDOW, 6)
    output_dims = 2 # Normal vs Abnormal
    EP = 30        
    LR = 1e-3 
    BS = 256
    save_path = 'best_model_binary.weights.h5'
    ...
    train_model(
        ...
        save_path, EP, LR, BS, 
        os.path.join(base_dir, "binary_cm_test.png"), 
        os.path.join(base_dir, "binary_cm_val.png")
    )
