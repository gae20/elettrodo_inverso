...
def build_unlabelled_limbs_dataset_with_patients(ids_list, h5_name, id_to_zip, add_gain=True):
    ...
    label_to_int = {lab: idx for idx, lab in enumerate(['normale'] + list(MAPPING_INV.keys()))}
                ...
                for inv_name in ACTIVE_SYNTH_CLASSES:
                    ...
                    if inv_name == 'ROT_ANTIORARIA':
                        ...
                        if np.random.random() < 0.5:
                            extra_mult = np.random.uniform(1.0, 1.5)
                        else:
                            extra_mult = np.random.uniform(2.0, 4.0)
...
if __name__ == "__main__":
    ...
    target_h5 = os.path.join(out_dir, "unlabelled_z_median_limbs_test_with_patients.h5")

    print(f"\n=== GENERAZIONE TEST SET CON ID PAZIENTI ===")
    print(f"Target: {target_h5}")
    
    build_unlabelled_limbs_dataset_with_patients(test_ids, target_h5, id_to_zip, add_gain=True)
    print("\nFatto.")
