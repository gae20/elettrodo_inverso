...
Valuta il modello semi-supervisionato (LDenseNet) di una determinata iterazione sul test set fisso:
1. Accetta l'argomento --iter (default 1) da riga di comando.
2. Carica test_small.h5.
3. Carica i pesi da results/model_iter_{iter}.weights.h5.
4. Esegue la valutazione stampando le metriche sul test set.
5. Salva la matrice di confusione in results/cm_iter_{iter}.png.
"""
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="Valuta il modello semi-supervisionato.")
    parser.add_argument("--iter", type=int, default=1, help="Iterazione del self-training da testare (default: 1)")
    ...
    print("=" * 60)
    print(f"TEST MODELLO SEMI-SUPERVISIONATO (ITERAZIONE {args.iter})")
    print("=" * 60)
    ...
    # 2. Carica modello e pesi dell'iterazione
    model = build_model((500, 6), 6)
    weights_filename = f'model_iter_{args.iter}.weights.h5'
    weights_path = os.path.join(RESULTS_DIR, weights_filename)

    if not os.path.exists(weights_path):
        print(f"ERRORE: I pesi {weights_path} non esistono. Esegui prima train_semi.py.")
        sys.exit(1)
    ...
    # 3. Valuta
    cm_path = os.path.join(RESULTS_DIR, f'cm_iter_{args.iter}.png')
    acc, auroc, auprc, metrics, cm = evaluate_on_test(X_test, Y_test, model, cm_path)

    print(f"\n[RISULTATI ITERAZIONE {args.iter}]")
