...
class FixedF1Score(tf.keras.metrics.F1Score):
    """Workaround: F1Score in Keras 2.15 fails when y_true shape is (None, None)
    because it comes from a Sequence generator. We pin the second dim explicitly."""
    def __init__(self, num_classes, **kwargs):
        super().__init__(**kwargs)
        self._num_classes = num_classes

    def update_state(self, y_true, y_pred, sample_weight=None):
        y_true = tf.ensure_shape(y_true, [None, self._num_classes])
...
def train_model(model, train_data, val_data, x_test_list, y_test_list, save_path, EP, LR, BS, pic_path_test, pic_path_val):
    ...
    ME = FixedF1Score(num_classes=6, average='macro', name='f1_score')
...
if __name__ == '__main__':
    ...
    dataset_path_val   = os.path.join(dataset_dir, "unlabelled_final_noise_limbs_val.h5")
    dataset_path_train = os.path.join(dataset_dir, "unlabelled_final_noise_limbs_train.h5")
    
    # Usiamo il test set di validazione reale standard (quello ricreato oggi)
    dataset_path_test = os.path.join(dataset_dir, "labelled_z_median_limbs_test_validation.h5")
    ...
    # Cartella per i pesi e i risultati
    out_weights_dir = os.path.join(base_dir, 'models')
    out_logs_dir = os.path.join(base_dir, 'logs')
    os.makedirs(out_weights_dir, exist_ok=True)
    os.makedirs(out_logs_dir, exist_ok=True)
    save_path = os.path.join(out_weights_dir, 'best_model_final_noise_limbs.weights.h5')
    ...
    # IMPORTANTE: shuffle=False per il training perché abbiamo già effettuato 
    # il global shuffle su disco in fase di generazione (fase 3/3). 
    # Leggere sequenzialmente dal file shuffleato eliminerà il blocco del disco!
    train_gen = H5DataGenerator(dataset_path_train, batch_size=BS, num_classes=output_dims, shuffle=False)
    ...
    train_model(
        ...
        save_path, EP, LR, BS, 
        os.path.join(out_logs_dir, "final_noise_cm_test.png"), 
        os.path.join(out_logs_dir, "final_noise_cm_val.png")
    )
